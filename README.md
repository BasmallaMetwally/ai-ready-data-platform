...
These charts are generated by the reproducible pipeline from the synthetic
source data. They give a quick visual check of the analytics outputs that the
ETL and DQ gate make available.

| Customer segmentation | Sales anomaly detection |
| --- | --- |
| ![Customer segments plotted from the e-commerce pipeline](outputs/customer_segments.png) | ![Daily sales and detected anomalies](outputs/daily_sales_with_anomalies.png) |

![Forecast compared with actual sales](outputs/forecast_actual_vs_predicted.png)

### CI evidence — PostgreSQL and MySQL service containers

![GitHub Actions run #8: all five jobs succeeded, including the PostgreSQL and MySQL database-target job](docs/screenshots/ci-green-run.png)

[Run #8 (green)](https://github.com/BasmallaMetwally/ai-ready-data-platform/actions/runs/35521538847)
starts PostgreSQL 16 and MySQL 8 service containers, then loads the same
e-commerce star schema into both targets. The screenshot is a captured record
of that successful run; the link remains the live, authoritative result.
